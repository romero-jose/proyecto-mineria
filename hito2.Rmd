...
```{r}
# Comunas segun sector
unique(matriz_2019.08$ComunaSubida)
unique(matriz_centronorte$ComunaSubida)
unique(matriz_sur$ComunaSubida)
unique(matriz_oriente$ComunaSubida)
unique(matriz_occidente$ComunaSubida)
```
...
datos.centronorte <- scale(matriz_centronorte[,4:11], center = TRUE, scale = TRUE)
head(datos.centronorte)
```

```{r}
library(factoextra)
fviz_nbclust(x = na.omit(datos.centronorte), FUNcluster = kmeans, method = "wss", k.max = 10, diss = get_dist(na.omit(datos.centronorte), method = "euclidean"), nstart=50)
```
...
k.means <- kmeans(x = na.omit(datos.centronorte), centers = 4, nstart = 25)
...
```{r}
fviz_cluster(object = k.means, data = na.omit(datos.centronorte), show.clust.cent = TRUE, ellipse.type = "euclid", repel = TRUE) +
...
pca.nci <- PCA(X = datos.centronorte, scale.unit = TRUE, ncp = 64, graph = FALSE)
